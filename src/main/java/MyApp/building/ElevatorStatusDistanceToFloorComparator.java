 ...
 */
record ElevatorStatusDistanceToFloorComparator(boolean goingUp, Floor floor) implements Comparator<ElevatorStatus>
{
    /**
     ...
     */
    ElevatorStatusDistanceToFloorComparator
    {
        if (floor == null)
     ...
     */
    @Override
    public int compare(final ElevatorStatus o1, final ElevatorStatus o2)
    { // ASC
        // queue count
        final int resultQueueCount = o1.getQueueCount() - o2.getQueueCount();
        if (resultQueueCount != 0)
            return resultQueueCount;

        // distance to src + brake distance
        final double distanceToSrcElevator1 = Math.abs(
           Math.abs(o1.getYPosition() + o1.getActualDirection() * o1.getBrakeDistance()) - floor.getYPosition());
        final double distanceToSrcElevator2 = Math.abs(
           Math.abs(o2.getYPosition() + o2.getActualDirection() * o2.getBrakeDistance()) - floor.getYPosition());
        final int resultDistanceToSrc = (int) distanceToSrcElevator1 - (int) distanceToSrcElevator2;
        if (resultDistanceToSrc != 0)
            return resultDistanceToSrc;

        // direction: same, still, reverse
        final int requestDirection = goingUp ? 1 : -1;
        ...
        final int resultVelocity = Math.abs((int) o1.getVelocity()) - Math.abs((int) o2.getVelocity());
        if (resultVelocity != 0)
